  ...
  return (
    <main className="detail-page">
      <header className="detail-header">
        <Link className="wordmark wordmark-dark" href="/"><span className="wordmark-mark" /><span>SPATIAL<br />ARCHIVE</span></Link>
        <Link className="back-link" href="/archive"><ArrowLeft size={15} /> BACK TO MAP</Link>
      ...
      <section className="detail-body">
        <div className="detail-visual" aria-label={`${record.title} 3D 기록 미리보기`}>
          ...
          <div className="detail-cloud">
            {Array.from({ length: 48 }).map((_, i) => <i key={i} style={{ "--i": i } as React.CSSProperties} />)}
          </div>
        ...
        <div className="detail-info">
          ...
          <div className="detail-actions">
            <Link className="primary-action" href={`/viewer/${record.id}`}><Box size={18} /> 3D 공간 입장 <ArrowRight size={16} /></Link>
